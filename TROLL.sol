...
contract TROLL is ERC20 {
    ...
    uint16 public transferTaxRate = 950;
    ...
    // The trading pair
    mapping(address => bool) public tokenSwapPair;
    // In swap and liquify
     ...
     */
    constructor() ERC20("TrollFace Defi", "TROLL") {
        _operator = tx.origin;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual checkWhale(recipient, amount) override{
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0 || _excludedFromFees[tx.origin]) {
            ...
            uint256 taxAmount = tokenSwapPair[recipient] ? amount.mul(transferTaxRate * 3).div(10000) : amount.mul(transferTaxRate).div(10000);
    ...
    function setSwapPair(address _address, bool _enabled) public onlyOperator {
        tokenSwapPair[_address] = _enabled;
        _excludedFromAntiWhale[_address] = _enabled;
    }
    ...
    function updateTokenSwapRouter(address _router) public onlyOperator {
        ...
        address _tokenSwapPair = ITokenFactory(tokenSwapRouter.factory()).getPair(address(this), tokenSwapRouter.WETH());
        emit TokenSwapRouterUpdated(msg.sender, address(tokenSwapRouter), _tokenSwapPair);
    }
