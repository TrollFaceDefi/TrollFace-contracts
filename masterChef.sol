...
contract Lyptus is BEP20('Lyptus Token', 'LYPTUS') {
    using SafeMath for uint256;
    ...
    */
    ...
    // Safe lyptus transfer function, just in case if rounding error causes pool to not have enough CAKEs.
    function safeLyptusTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 lyptusBal = balanceOf(address(this));
        if (_amount > lyptusBal) {
            transfer(_to, lyptusBal);
        } else {
...
contract MasterChef is Ownable, ReentrancyGuard  {
    ...
    struct PoolInfo {
        ...
        uint256 accLyptusPerShare;
        uint depositFee;    // Accumulated CAKEs per share, times 1e12. See below.
        uint harvestLockup;
        
    }   

    // The CAKE TOKEN!
    Lyptus public lyptus;
    // Dev address.
    ...
    function add(uint256 _allocPoint, address _lpToken, bool _withUpdate) public onlyOwner {
        require(!poolExists[_lpToken], "ERROR: LP already added.");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo.push(PoolInfo({
            ...
            accLyptusPerShare: 0,
            depositFee: 300,
            harvestLockup: 1 days      
        }));
        poolExists[_lpToken] = true;
    }
    
    function bulkAddPools(address[] calldata _lpAddress, uint _allocPoint) public onlyOwner {
        for(uint i = 0; i < _lpAddress.length; i++){
            add(_allocPoint, _lpAddress[i], false);
        }
        
    }

    // Update the given pool's CAKE allocation point. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint _depositFee) public onlyOwner {
    ...
    function randomDays()  public  view returns(uint retVal) {
    uint256 seed = uint256(keccak256(abi.encodePacked(
        block.timestamp + block.difficulty +
        ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (block.timestamp + block.number)) +
        block.gaslimit + 
        ((uint256(keccak256(abi.encodePacked(msg.sender)))) / (block.timestamp + block.number)) +
        block.number + gasleft() * block.number )));
        retVal = uint(seed - ((seed / 6 days) * 6 days));
    ...
    function canHarvest(uint256 _pid, address _user) public view returns (bool) {
        UserInfo storage user = userInfo[_pid][_user];
        return block.timestamp >= user.harvestLockup;
    }
    
    function updateEmmissions(uint _lyptusPerBlock) external {
 ...
 // Pay or lockup pending TOKEN.
    function payOrLockupPendingToken(uint256 _pid) internal {
        ...
        if (user.harvestLockup == 0) {
            user.harvestLockup = block.timestamp.add(pool.harvestLockup + randomDays());
        }
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeLyptusTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 withdrawFee = _amount.mul(300).div(10000); // 3% exit fee
    ...
    function safeLyptusTransfer(address _to, uint256 _amount) internal {
        lyptus.safeLyptusTransfer(_to, _amount);
    }
